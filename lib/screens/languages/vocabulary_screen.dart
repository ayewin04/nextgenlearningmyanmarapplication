...
class _VocabularyScreenState extends State<VocabularyScreen> {
  ...
  List<ExamVocabularyModel> _searchResults = [];
  ...
  // Cache for page data
  final Map<int, List<ExamVocabularyModel>> _pageCache = {};
  ...
  Future<void> _estimateTotalCount() async {
    ...
    try {
      ...
      print('📊 Estimated total: $_estimatedTotal words, $_totalPages pages');
    } catch (e) {
  ...
  Future<void> _searchVocabulary(String query) async {
    ...
    try {
      final results = await _firestoreService.searchVocabulary(
  ...
  Future<void> _loadVocabulary({bool refresh = false, int? targetPage}) async {
    ...
    try {
      if (targetPage != null && _pageCache.containsKey(targetPage)) {
        ...
        _vocabularies = _pageCache[targetPage]!;
      ...
      if (targetPage != null && targetPage > 0 && startAfter == null) {
        ...
        for (int i = 0; i <= targetPage; i++) {
          ...
          final vocabList = newData.map((data) {
          ...
          final uniqueList = _removeDuplicatesByWordExamLevel(vocabList);
          _pageCache[i] = uniqueList;
          
          if (i == targetPage) {
            _vocabularies = uniqueList;
            _currentPage = i;
            print('✅ Loaded page $i with ${uniqueList.length} items');
          }
      ...
      if (refresh) {
        _vocabularies = uniqueList;
        _currentPage = 0;
        _pageCache[0] = uniqueList;
      } else if (targetPage != null && targetPage > 0) {
        _vocabularies = uniqueList;
        _currentPage = targetPage;
        _pageCache[targetPage] = uniqueList;
      } else {
        _vocabularies = uniqueList;
        _currentPage++;
        _pageCache[_currentPage] = uniqueList;
      }
      
      if (_estimatedTotal > 0) {
        _totalPages = (_estimatedTotal / _pageSize).ceil();
      } else {
        _totalItems = _vocabularies.length + (_hasMoreData ? _pageSize : 0);
        _totalPages = (_totalItems / _pageSize).ceil();
      }
      
      print('✅ Loaded ${_vocabularies.length} words (Page ${_currentPage + 1}/$_totalPages)');
  ...
  Future<void> _loadPage(int page) async {
    ...
    await _loadVocabulary(refresh: true, targetPage: page);
  }
  ...
  List<ExamVocabularyModel> get _displayList {
    if (_isSearching && _searchQuery.isNotEmpty) {
        ...
        actions: [
          if (_isSearching && _searchQuery.isNotEmpty)
            ...
            Container(
              ...
              child: Text(
                '${_estimatedTotal > 0 ? _estimatedTotal : _vocabularies.length}',
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 12,
                  fontWeight: FontWeight.bold,
